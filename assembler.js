...
function SimulatorWidget(node) {
  ...
  function UI() {
   ...
   $(document).ready(function () {
    ...
    // Create the project dropdown and add it to the buttons area
    const projectSelect = $('<select class="projectSelect"></select>');
    projectSelect.append('<option value="">Select Project</option>');
    $('.saveButton').after(projectSelect);

    // Create the delete button
    const deleteButton = $('<input type="button" value="Delete" class="deleteButton" />');
    projectSelect.after(deleteButton);
    ...
    $('.saveButton').click(function () {
        ...
        if (projectName) {
            localStorage.setItem(`project_${projectName}`, code);
    ...
    projectSelect.change(function () {
        ...
        if (projectName) {
            const code = localStorage.getItem(`project_${projectName}`);
            if (code !== null) {
                codeArea.val(code);
    ...
    deleteButton.click(function () {
        ...
        if (projectName && confirm(`Are you sure you want to delete project '${projectName}'?`)) {
  ...
  function Display() {
    ...
    function updatePixel(addr) {
        ...
        // Use the full 8 bits of the value for color
        const r = (value >> 5) & 0x07;
        const g = (value >> 2) & 0x07;
        const b = value & 0x03;
        const color = `rgb(${r * 36}, ${g * 36}, ${b * 85})`;
        const y = Math.floor((addr - 0x1000) / numX);
        const x = (addr - 0x1000) % numX;
        ctx.fillStyle = color;
